                            ...
                            <div class="col-xs-12 bottom text-center">
                                <div class="col-xs-6 col-sm-6 emphasis" style="width: 100%;">
                                    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                                        ...
                                        <a href="{{ url('usuario') }}"><button type="button" class="btn btn-default btn-xs">Regresar</button></a>
                                        
                                        @foreach($personal as $per)
                                            <a href="{{ route('Ficha-usuario', $per->id) }}" target="_blank"><button type="button" class="btn btn-info btn-xs" data-toggle="tooltip" data-placement="top" title="PDF"><i class="fa fa-file-pdf-o"></i></button></a>
                                            <a href="{{ route('usuario.edit', $per->id) }}"><button type="button" class="btn btn-success btn-xs" data-toggle="tooltip" data-placement="top" title="Editar"><i class="fa fa-edit"></i></button></a>
                                        @if ($per->eliminar == true)
...
<script>
        ...
        window.location = "{{url('/')}}" + "/" + id + "/deshabilitar/usuario";
        });
        };
        ...
        window.location.href = "{{url('/')}}" + "/" + id + "/habilitar/usuario";
        });
        };
        //    Esto script es para Eliminar un personal
        var eliminar = function (id) {
        $.alertable.confirm("¿Esta seguro de Eliminar el registro?").then(function () {
        window.location.href = "{{url('/')}}" + "/" + id + "/destroy/usuario";
        });
        };
        //Agregando atributos a la navbar para que este activa
        $('#componentes-ul-usu').attr('style','display: block');
        $("#componentes-li-usu").addClass("active");
</script>
@endsection()